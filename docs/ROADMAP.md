...
## v2.0 Roadmap — RKS-Governed Graduated Autonomy

**Status:** v1.0 complete (27 phases, 108 plans). v2.0 planning begins from this baseline.

**Governing constraint:** v2.0 phases must be sequenced against the Reflexive Knowledge System
(RKS) bootstrap. OPE is the first confirmed RKS-owning project in the ecosystem. Protocol
compliance is a prerequisite for graduated autonomy — the Governing Orchestrator cannot
autonomously govern OPE without querying its epistemic status via the RKS protocol.

Full RKS architecture, build plan, and ecosystem inventory:
`/Users/david/projects/reflexive-knowledge-system/design/`

### v2.0 Phase 1: RKS Protocol Compliance
**Goal:** OPE exposes the four RKS protocol operations over its existing Layer 1–2 infrastructure
(DuckDB + JSONL). OPE becomes the first registered project in the ecosystem.

- Implement Conformance Probe, Epistemic Status Query, Knowledge Retrieval, Evidence Write
- Upgrade the `memory_candidates` → MEMORY.md flow to protocol-compliant Evidence Write
- Register OPE with the Governing Orchestrator

**Gate:** `GOVERNANCE_PULSE("orchestrator-policy-extraction")` returns a valid Epistemic Status
response. Evidence Write accepts and stores one agent-produced artifact with a valid
ground_truth_pointer.

**Reference:** RKS BUILD-PLAN.md Phases 0–1:
`/Users/david/projects/reflexive-knowledge-system/design/BUILD-PLAN.md`

### v2.0 Phase 2: Layer 3–4 Upgrade
**Goal:** OPE gains Semantic Validation (Layer 3) and Transformation (Layer 4). MEMORY.md CCDs
become Layer 3 validation rules. The Epistemic Status Query returns meaningful constraint
violation counts and coverage percentage.

- Implement Layer 3: validation rules derived from OPE's active MEMORY.md CCDs
- Evidence Write rejects artifacts failing Layer 3 with a specific constraint_id
- Implement Layer 4: graph queries exposing the episode → constraint causal chain

**Gate:** GOVERNANCE_PULSE returns `coverage_pct > 50%` and a live `constraint_violations` count.

### v2.0 Phase 3: Canon Registration
**Goal:** OPE deposits `project_status` and `constraint_violations` Snippet Records into Canon.
The GO can query OPE's epistemic status through Canon without project-specific code.

**Gate:** Canon Knowledge Retrieval query returns ranked Snippet Records for OPE with valid
ground_truth_pointers.

### v2.0 Phase 4: Graduated Autonomy
**Prerequisite:** Phases 1–3 complete. The GO can query OPE's epistemic status before deciding
whether to intervene.

- Preference model training on v1.0 episode history
- Shadow mode validation: GO decisions agree with human decisions ≥70%
- Graduated autonomy gates: low-risk tasks autonomous, high-risk tasks human-gated

### v2.0 Phase 5: OpenClaw Integration
**Goal:** OpenClaw (macro persistence) integrated with the RKS protocol. `run_id` generated by
GO at dispatch and registered across all sessions in all repos.

### v2.0 Phase 6: Preference Model Training
**Goal:** Preference model trained on labeled episodes. Auto-approval of low-risk tasks
operational.
